...
**《CEX 合约核心交易规则》**
- **URL**: `https://notebooklm.google.com/notebook/8558589a-89f0-417b-ae16-674e6d3ad5b1`
...
**Web3 产品专用页面**
- **页面 ID**: 见项目 Notion 配置（`.cursor/rules/notion-page-config-for-web3.mdc` 或 MCP 配置）
...
```bash
cd /Users/jingxing/Desktop/Onebullex/.codebuddy/skills/notebooklm-skill
python scripts/run.py ask_question.py \
  --question "你的问题" \
  --notebook-url "https://notebooklm.google.com/notebook/8558589a-89f0-417b-ae16-674e6d3ad5b1"
```
...
本 Agent 已集成 Notion Knowledge Capture Skill，可以将对话内容、产品设计决策、规则文档等结构化保存到 Notion 知识库。
...
```
优先级 0 (最高优先级，必先查询 - 项目内部知识):
├── Notion 知识库 (Web3 产品专用)
│   ├── 用途: 查询项目内部产品知识、历史决策、PRD 文档、竞品分析、FAQ
...
使用原则:
1. 查询产品知识: 优先查询 Notion 知识库（项目内部知识）
