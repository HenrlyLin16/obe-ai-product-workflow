#!/usr/bin/env bash
# Sync the repo source of onebullex-android-qa to local Codex and Cursor mirrors.

set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SOURCE="${ROOT}/workflow/skills/qa/onebullex-android-qa"
CODEX_DEST="${HOME}/.codex/skills/onebullex-android-qa"
CURSOR_DEST="${ROOT}/.cursor/skills/onebullex-android-qa"
...
usage() {
  cat <<'EOF'
Usage: scripts/sync-onebullex-android-qa-skill.sh [options]

Options:
  --dry-run      Show sync operations without writing files.
  --check        Verify source and mirrors are in sync.
  --codex-only   Sync/check only ~/.codex/skills/onebullex-android-qa.
  --cursor-only  Sync/check only .cursor/skills/onebullex-android-qa.
...
sync_dest() {
  ...
  if [[ "$CHECK" -eq 1 ]]; then
    local changes
    changes="$(rsync -r --checksum --delete --dry-run --itemize-changes "${RSYNC_EXCLUDES[@]}" "${SOURCE}/" "${dest}/" | grep -v '^\\.d' || true)"
    if [[ -n "$changes" ]]; then
      printf '%s\n' "$changes"
      return 1
    fi
