...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Review and apply confirmed onebullex-web-qa learning candidates.")
    parser.add_argument("evidence_dir", help="Evidence directory from a onebullex-web-qa run.")
...
def render_patch(evidence_dir: Path, source_path: Path, confirmed: list[dict[str, Any]]) -> str:
    ...
    lines.append("- Keep Codex and Cursor mirrors generated through `scripts/sync-onebullex-web-qa-skill.sh`.\n")
    return "".join(lines)
...
def main() -> int:
    ...
    if args.write_skill:
        ...
        print("Next: run scripts/sync-onebullex-web-qa-skill.sh from the repo root.")
