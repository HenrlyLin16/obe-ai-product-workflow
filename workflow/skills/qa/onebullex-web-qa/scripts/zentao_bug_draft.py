#!/usr/bin/env python3
"""Convert confirmed OneBullEx Web QA product-bug candidates into Zentao drafts."""
...
SAMPLE = [
    {
        ...
        "title": "[Web][OneBullEx testnet] Change% sorting does not toggle descending",
        "module": "/",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("template", nargs="?", help="Path to confirmed-bugs.template.json or onebullex-web-qa-report.json.")
...
def report_results_to_items(data: dict[str, Any], path: Path) -> list[dict[str, Any]]:
    ...
    for result in data.get("results", []):
        ...
        items.append({
            ...
            "bug_candidate": True,
            "category": "product_bug",
            "title": f"[Web][OneBullEx testnet] {result.get('flow')}/{result.get('step')} failed",
            "module": "/",
            "affected_version": "",
            "bug_type": "代码错误",
            "severity": 3,
